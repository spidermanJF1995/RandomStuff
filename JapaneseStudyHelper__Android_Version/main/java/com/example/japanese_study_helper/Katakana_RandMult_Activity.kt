...
class Katakana_RandMult_Activity : AppCompatActivity()
{
    ...
    // Because "allKatakanaCharacters" has its contents being removed using "removeAt[currentCharacter]", it is being re-indexed
    // The re-indexing breaks the attachment between the value at "allKatakanaCharacters[currentCharacter]" and the answer associated with
    // it at "allRanjiCharacters[currentCharacter]"
    // Therefore, the solution is to have "allRanjiCharacters" get re-indexed as well
    // But, this poses the problem of removing possible wrong answer choices from the multiple choice buttons
    // This variable will hold the possible wrong answer choices so all Ranji characters have a chance to be the wrong answer choice
    // for all katakanaCharacters...
    private var allRanjiCharactersHolder = mutableListOf<String>()

    // Used to track the missed characters to help display them later...
    private var allMissedKatakanaCharacters = mutableListOf<String>()
    private var allMissedRanjiCharacters = mutableListOf<String>()
    private var KatakanaCharacter = ""
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_katakana_rand_mult)
        ...
        val correctScore: TextView = findViewById(R.id.KatakanaRandMultDisplayCorrectNumberValue)
        val wrongScore: TextView = findViewById(R.id.KatakanaRandMultDisplayWrongNumberValue)
        correctScore.text = "0"
        wrongScore.text = "0"

        readFiles()

        countToEndOfGame = allKatakanaCharacters.size
    ...
    private fun readFiles()
    {
        ...
        try {
            ...
            allRanjiCharactersHolder.addAll(allRanjiCharacters)
        } catch (e: IOException){
    ...
    private fun assignAnswers()
    {
        ...
        var buttonCopy: Button = findViewById(R.id.KatakanaRandMultButtonTopLeft)
        buttons.add(buttonCopy)

        buttonCopy = findViewById(R.id.KatakanaRandMultButtonTopRight)
        buttons.add(buttonCopy)

        buttonCopy = findViewById(R.id.KatakanaRandMultButtonBottomLeft)
        buttons.add(buttonCopy)

        buttonCopy = findViewById(R.id.KatakanaRandMultButtonBottomRight)
        buttons.add(buttonCopy)

        // Gets a random character...
        currentCharacter = Random.nextInt(0, allKatakanaCharacters.size)

        // Store the current character and answer...
        KatakanaCharacter = allKatakanaCharacters[currentCharacter]
        ranjiCharacter = allRanjiCharacters[currentCharacter]

        // Makes sure no characters show up twice...
        allKatakanaCharacters.removeAt(currentCharacter)
        allRanjiCharacters.removeAt(currentCharacter)
        ...
        var labelCopy: TextView = findViewById(R.id.KatakanaRandMultDisplayChar)
        labelCopy.text = KatakanaCharacter
        ...
        buttons.forEach{
            ...
            do
            {
                randomCharacter = Random.nextInt(0, allRanjiCharactersHolder.size)
            }
            while(usedChoices.contains(allRanjiCharactersHolder[randomCharacter]))

            // keep track of another used character choice...
            usedChoices.add(allRanjiCharactersHolder[randomCharacter])

            // make the random choice be the text of a button...
            it.text = allRanjiCharactersHolder[randomCharacter]
        } // end forEach{}
    ...
    fun checkAnswerOnClick(view: View)
    {
        ...
        else
        {
            ...
            allMissedKatakanaCharacters.add(KatakanaCharacter)
    ...
    fun continueButtonClick(view: View)
    {
        ...
        if(countToEndOfGame == 0)
        {
            ...
            if(allMissedKatakanaCharacters.size == 0)
            {
            ...
            else
            {
                ...
                intent.putStringArrayListExtra("MissedJapaneseCharacterList", ArrayList(allMissedKatakanaCharacters))
...
} // end Katakana_RandMult_Activity